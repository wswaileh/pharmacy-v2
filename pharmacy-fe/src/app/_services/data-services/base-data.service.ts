...
export class BaseDataService<Entity extends IBaseModel<IBaseDTO, IBaseDTO>> extends DefaultDataService<Entity> {
  ...
  getAll(): Observable<Entity[]> {
    return super.getAll().pipe(map(entities =>
      entities.map(entity => new this.entityType().fromDTO(entity))
    ));
  }

  getById(id: string | number): Observable<Entity> {
    return super.getById(id).pipe(map(entity => new this.entityType().fromDTO(entity)));
  }

  add(entity: Entity): Observable<Entity> {
    return super.add(entity).pipe(map(newEntity => {
      if (newEntity) {
